...
#                continue
        ...
        for i in num_int:
            c = lines[i].split()
            if c[2] == 'BOND':
                key = ('r', '('+c[4][2:]+'-'+c[6][2:]+')'+'/\AA')
                int_coord[key] = float(c[3])
                keys.append(key)
            if c[2] == 'ANGLE':
                key = ('\\theta', '('+c[4][2:]+'-'+c[6][2:]+'-'+c[8][2:]+')'+'/\degree')
                int_coord[key] = float(c[3])
                keys.append(key)
            if c[2] == 'LINEAR':
                if len(c) == 10:
                    key = ('\\theta', '('+c[5][2:]+'-'+c[7][2:]+'-'+c[8][2:]+')'+'/\degree')
                    int_coord[key] = float(c[4])
                    keys.append(key)
                else:
                    key = ('\Tau', '('+c[5][2:]+'-'+c[7][2:]+'-'+c[8][2:]+'-'+c[9][2:]+')'+'/\degree')
                    int_coord[key] = float(c[4])
                    keys.append(key)
        ...
        if num_dtj != 0:
            ...
            for plus in range(20):
                ...
                if len(c) == 0: break
                key = c[0]+c[1]
                dtj[key] = map(float,c[-2:])
        else:
            print spectrof + " Error: cannot find Constants in A-reduced Hamiltonian DELTA J!"
            dtj = 'None'
        ...
        if num_freq != 0:
            ...
            print spectrof + " Error: cannot find harmonic and fundamental frequencies!"
            freqs = 'None'
   ...
   #             eigen_val = map(str,lines[num_eig].split()) # in case there is *** in line ##
            ...
            print spectrof + " Error: cannot find Eigen values!"
            eigen_val = 'None'
...
#                vib_state[key].append(map(float, c[-3:]))   # [Requil, Rg, Ralpha]
		try:
		    ...
		    vib_state[key].append(map(str, c[-3:]))	# in case there is *** in line ##
