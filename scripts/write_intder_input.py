...
#    iopt = [n1,n2,n3,0,0,0,0,n8,0,0,0,0,0,0,0,0]
    ...
    if output == 'intder.in':
        ...
        sys.exit()
    n1 = xyz_bohr.shape[0]
    As = raw_input("Is this molecule linear? Yes or No ")
    if As == 'Yes':
        n2 = 3*n1-5
    else:
        n2 = 3*n1-6
...
#        f.write('        ')
    ...
    if output == 'intder_freqs.in':
        atoms = raw_input('Input atoms with math, put space between two atoms: ')
        for atom in atoms.split():
            f.write('%12s'%atom)
        f.write('\n')
        ffdir = raw_input('Input the directory where file fort.9903 locates: ')
